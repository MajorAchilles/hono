...
const guestServer = {
  ...
  initializeState: () => {
    ...
    config.guests.forEach((guest) => {
      servers[guest.id] = {
        ...
        matchHost: guest.host,
        targetPort: guest.target.port,
        targetHost: guest.target.host,
        dir: guest.dir,
  ...
  getTargetUrl: (host, port) => `http://${host}${port ? `:${port}` : ""}`,

  getPortMap: () => {
    const serverList = guestServer.getServerList();
    const map = {};

    serverList.forEach((server) => {
      map[server.matchHost] = guestServer.getTargetUrl(server.targetHost, server.targetPort);
    });
  ...
  getServerList: () => {
    ...
    return Object.keys(honostate.servers).map((id) => {
      return {
        ...
        targetPort: honostate.servers[id].targetPort,
        targetHost: honostate.servers[id].targetHost,
        matchHost: honostate.servers[id].matchHost,
