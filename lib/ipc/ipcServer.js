      ...
      Object
        ...
        .forEach((eventName) => {
          ipc.server.on(eventName, (data, socket) => {
            ...
            const sendTable = (message) => {
              ipc.server.emit(socket, EVENTS.IPC.TABLE, message);
            };

            const sendGeneric = (message, eventType) => {
              ipc.server.emit(socket, eventType || EVENTS.IPC.MESSAGE, message);
            };
